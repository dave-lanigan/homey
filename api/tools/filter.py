...
async def get_listing_details_async(page, listing_url: str) -> dict:
    """
    ...
    try:
        ...
        await page.wait_for_selector("h1, main", timeout=5000)
    except Exception:
        print("   ⚠️ Navigation timeout, continuing anyway...")
...
async def scrape_listings_concurrent(
    ...
    """
    Scrape listing details concurrently using async Playwright.

    Args:
        context: Async Playwright browser context
    ...
    async def scrape_with_semaphore(url, index):
        ...
        async with semaphore:
            ...
            try:
                ...
                details = await get_listing_details_async(page, url)
...
class AirbnbFilters(BaseModel):
    ...
    keywords: Optional[list[str]] = Field(default=None, description="Custom keywords to search in listing text")
    match_all_keywords: bool = Field(default=False, description="Match ALL keywords (default: match ANY)")
    amenities: Optional[list[str]] = Field(default=None, description="Airbnb amenity names (e.g. pool, gym, wifi)")
    ...
    max_listings: int = Field(default=50, ge=1, description="Max listings to scrape")
    ...
    top_k: int = Field(default=10, ge=1, description="Number of ranked results to return from the semantic search")
...
class FilterUpdate(BaseModel):
    ...
    keywords: Optional[list[str]] = Field(default=None, description="Keywords to add to the search")
    match_all_keywords: Optional[bool] = Field(default=None, description="Match ALL keywords instead of ANY")
    amenities: Optional[list[str]] = Field(
        default=None,
        description="Amenity names to add (wifi, kitchen, washer, dryer, air_conditioning, heating, pool, hot_tub, gym, free_parking, ev_charger, crib, bbq_grill, breakfast, fireplace, workspace, tv, pets_allowed, smoking_allowed, wheelchair_accessible, elevator, beach_access, waterfront, self_checkin)",
    )
    min_price: Optional[int] = Field(default=None, ge=0, description="Minimum price per night in USD")
    max_price: Optional[int] = Field(default=None, ge=0, description="Maximum price per night in USD")
...
def format_listings_for_chat(response: AirbnbSearchResponse, top_k: int = 10) -> str:
...
def build_parser():
    ...
    # Pagination
    parser.add_argument("--max-listings", type=int, default=50, help="Max listings to scrape")
